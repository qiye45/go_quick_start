...
func main() {
	...
	portEnd := flag.Int("portEnd", 10000, "the port on which the scanning ends")
	timeout := flag.Duration("timeout", time.Millisecond*100, "timeout")
	workerNum := flag.Int("workerNum", 200, "the number of worker")
	flag.Parse()
