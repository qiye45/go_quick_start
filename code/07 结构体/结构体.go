...
func main() {
	...
	*	{lat:4.215 long:135.512}
	...
	//fmt.Println(bytesV1, errV1) //nil表示无错误
	exitOnError(errV1)
	fmt.Println("bytesV1", string(bytesV1)) //{}

	spiritV2 := locationV2{Lat: 12.433, Long: 144.843}
	bytesV2, errV2 := json.Marshal(spiritV2)
	exitOnError(errV2)
	fmt.Println("bytesV2", string(bytesV2)) //{"Lat":12.433,"Long":144.843}

	spiritV3 := locationV3{Lat: 12.433, Long: 144.843}
	bytesV3, errV3 := json.Marshal(spiritV3)
	exitOnError(errV3)
	fmt.Println("bytesV3", string(bytesV3)) //{"latitude":12.433,"longitude":144.843}
}
